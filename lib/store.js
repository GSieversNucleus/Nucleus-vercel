...
const K_PUSH_SUBS = 'nucleus:push:subs'; // encrypted envelope of { endpoint: {teamMemberId, keys, userAgent, createdAt} }
const K_RAKEN_TOKENS = 'nucleus:raken:tokens'; // encrypted envelope of {accessToken, refreshToken, expiresAt, connectedBy, connectedAt} — OAuth credentials for the company's Raken account, so ALWAYS encrypted, never plain
const K_GEOCODE_CACHE = 'nucleus:geocode:cache'; // plain (unencrypted — coordinates for a job's own address aren't sensitive) JSON map of normalized address -> {lat, lng, displayName}
const K_GEOCODE_LASTCALL = 'nucleus:geocode:lastcall'; // plain epoch-ms string — shared across serverless instances so they all back off the same free Nominatim service together, not each thinking it's the only caller
...
// ---- Push subscriptions (push-subscriptions.json equivalent) — kept as
// their own Redis key, the same reasoning as users/sessions living apart
// from the main state document: a subscription is a per-device thing tied
// to one person's browser, not part of the shared job data the
// compare-and-swap above protects, so it has no business going through
// that same version-checked save path. Keyed by endpoint (the push
// service's own unique URL for that one browser installation). ----
/* ---- Raken OAuth tokens. One connection per Nucleus deployment (Raken is
   the company's account, not a per-person one), stored through the same
   encrypted envelope as everything else — a refresh token is valid for 180
   days, so it is exactly the kind of thing that must never sit in plain text
   in a Redis backup. ---- */
async function loadRakenTokens() {
  const raw = await redis.get(K_RAKEN_TOKENS);
  return decryptEnvelope(raw, null);
}
async function saveRakenTokens(tokens) {
  await redis.set(K_RAKEN_TOKENS, encryptEnvelope(tokens));
}
async function clearRakenTokens() {
  await redis.del(K_RAKEN_TOKENS);
}

async function loadPushSubs() {
  const raw = await redis.get(K_PUSH_SUBS);
  return decryptEnvelope(raw, {});
}
async function savePushSubs(subs) {
  await redis.set(K_PUSH_SUBS, encryptEnvelope(subs));
}
async function addPushSub(endpoint, data) {
...
// ---- Geocode cache + shared throttle for the Jobs Map — see lib/geocode.js
// for the actual Nominatim call this backs. Kept in Redis rather than an
// in-memory Map/variable for the same reason everything else in this file
// is: a serverless function has no reliable memory between invocations, and
// two cold instances geocoding at once would otherwise have no way to know
// about each other and could together exceed Nominatim's 1-request/second
// policy. Not encrypted — a street address turned into a lat/lng is not
// sensitive data, and this is a small, low-value cache, not app data. ----
function parseJsonish(raw, fallback) {
  if (raw == null) return fallback;
  if (typeof raw !== 'string') return raw;
  try { return JSON.parse(raw); } catch (e) { return fallback; }
}
async function getGeocodeCacheEntry(key) {
  const map = parseJsonish(await redis.get(K_GEOCODE_CACHE), {});
  return map[key] || null;
}
async function setGeocodeCacheEntry(key, value) {
  const map = parseJsonish(await redis.get(K_GEOCODE_CACHE), {});
  map[key] = value;
  await redis.set(K_GEOCODE_CACHE, JSON.stringify(map));
}
async function getGeocodeLastCallAt() {
  const raw = await redis.get(K_GEOCODE_LASTCALL);
  return Number(raw || 0);
}
async function setGeocodeLastCallAt(ms) {
  await redis.set(K_GEOCODE_LASTCALL, String(ms));
}
...
module.exports = {
  ...
  getGeocodeLastCallAt,
  setGeocodeLastCallAt,
  MIN_PASSWORD_LEN: 8
};
